...
5. ast2graph統合機能の実装【最優先】
   - store_ast_graphメソッドの実装
   - ast2graph出力形式の検証機能
   - グラフデータのバリデーション
   - バッチインポートの最適化（10,000ノード/秒）

6. intent_store連携機能の実装
   - link_intent_to_astメソッドの実装
   - 意図-ASTマッピングテーブルの設計
   - 関連データの整合性保証
   - pgvectorを使用したベクトル検索の実装

7. code_intent_search統合機能の実装
   - 統合検索APIの実装
   - グラフ・ベクトル・全文検索の組み合わせ
   - ランキング機能の実装
...
2. **ast2graph連携API (store_ast_graph)**
   - ASTグラフデータの受け取りと検証
   - Neo4jへの効率的なバッチインポート（10,000ノード/秒以上）
   - グラフ構造の整合性チェック
   - ノードタイプ: ASTNode（id, node_type, value, lineno, source_id）
   - エッジタイプ: CHILD, NEXT, DEPENDS_ON
